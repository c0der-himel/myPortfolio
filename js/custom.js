...
front.addEventListener('click', function (e) {
  for (let i = 0; i < frontP.length; i++) {
    ...
    frontP[i].classList.add('d-block');
  }
  for (let i = 0; i < fullP.length; i++) {
    fullP[i].classList.add('d-none');
  }
  for (let i = 0; i < backP.length; i++) {
    backP[i].classList.add('d-none');
  }
});
...
back.addEventListener('click', function (e) {
  for (let i = 0; i < backP.length; i++) {
    ...
    backP[i].classList.add('d-block');
  }
  for (let i = 0; i < frontP.length; i++) {
    frontP[i].classList.add('d-none');
  }
  for (let i = 0; i < fullP.length; i++) {
    fullP[i].classList.add('d-none');
  }
